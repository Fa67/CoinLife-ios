                    ...
                    if ( coin_kind[i][1] == "BitFlyer"){
                        ...
                        let task = URLSession.shared.dataTask(with: url! as URL) { data, response, error in
                            ...
                            if (text.contains("\"ltp\":") && coin_kind[i][0] == "BTC"){
                                ...
                                //coin_kind[i][3] = "미지원"
                                coin_kind[i][2] = String(Int(Float(coin_kind[i][2])! * Float(TodayViewController.usd)!))
                                ...
                                let task = URLSession.shared.dataTask(with: url! as URL) { data, response, error in
                                    ...
                                    if (text.contains("\"ltp\":")){
                                        coin_kind[i][2] = text.components(separatedBy: "\"ltp\":")[1].components(separatedBy: ",")[0]
                                        //coin_kind[i][3] = "미지원"
                                        coin_kind[i][2] = String(Int(Float(coin_kind[i][2])! * Float(TodayViewController.usd)! * Float(btc_tmp!)))
